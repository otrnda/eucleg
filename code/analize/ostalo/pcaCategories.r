...
dataForPCA <- NULL
dataForPCA <- readExcelFajl(1, c("character", rep("double", 9)), "allBlupsV6.xlsx")
dataSaKategorijama <- readExcelFajl(1, c("character", "character", "integer", "character", "integer","character", "integer"), "allDataV6categories.xlsx")
dataSaKategorijama <- dataSaKategorijama[ ,1:7]

dataForPCAplusKat <- merge(dataForPCA, dataSaKategorijama[,c(1,4)], by.x="EUCID", by.y="EUCID")
View(dataForPCAplusKat)

source("loader.r"); pcaRez <- pcaSakategorijamaBezLabela(dataForPCAplusKat, "pcaResults")
# View(dataForPCA)
# View(pcaBezKategorija)
# View(pcaSakategorijama)
# View(dataForPCA)
